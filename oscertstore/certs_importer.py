...
def run(plugin):
    ...
    if QgsAuthManager.instance().isDisabled():
        plugin.log(QgsAuthManager.instance().disabledMessage())
        return False
    ...
    plugin.log(plugin.tr("Number of possible CAs found: {0}").format(len(ca_pems)))
    ...
    for ca_cn, ca_pem in ca_pems.items():
        ...
        if not cas:
            plugin.log(plugin.tr("Could not convert PEM to QSslCertificate: {0}").format(ca_cn))
            continue

        ca = cas[0]
        if not ca.isValid():
            plugin.log(plugin.tr("  cert not valid: {0}").format(ca_cn))
            continue
        if not QgsAuthCertUtils.certificateIsAuthority(ca):
            plugin.log(plugin.tr("  cert not a CA: {0}").format(ca_cn))
            continue
        if ca in trusted_cas:
            plugin.log(plugin.tr("  cert already in trusted CA cache: {0}").format(ca_cn))
            continue
